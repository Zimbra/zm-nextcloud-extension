...
my %PKG_GRAPH = (
   "zimbra-mbox-nextcloud-service" => {
      summary   => "Zimbra Nextcloud Service",
      version   => "$version",
      revision  => $revision,
      hard_deps => [],
      soft_deps => [],
      ...
      stage_fun  => sub { &stage_zimbra_nextcloud_service(@_); },
   },
...
sub stage_zimbra_nextcloud_service($)
{
   my $stage_base_dir = shift;
    ...
    cpy_file( "zimlet/zm-x-zimlet-nextcloud.zip",    "$stage_base_dir/opt/zimbra/zimlets-network/zm-x-zimlet-nextcloud.zip" );
